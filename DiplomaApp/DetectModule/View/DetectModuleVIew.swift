...
class ViewController: UIViewController, ARSCNViewDelegate {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        Observable<Int>.interval(1.0, scheduler: SerialDispatchQueueScheduler(qos: .default))
            ...
            .subscribe { [unowned self] _ in
                self.foundFaces.filter{ $0.updated.isAfter(seconds: 1.5) && !$0.hidden }.forEach{ face in
    ...
    private func createfaceObservable(face: VNFaceObservation, image: CIImage, frame: ARFrame) -> Observable<(classes: [VNClassificationObservation], position: SCNVector3, frame: ARFrame)> {
        return Observable<(classes: [VNClassificationObservation], position: SCNVector3, frame: ARFrame)>.create{ observer in
            ...
            guard let worldCoord = self.normalizeWorldCoord(boundingBox) else { return Disposables.create() }
    ...
    private func updateNode(classes: [VNClassificationObservation], position: SCNVector3, frame: ARFrame) {
        ...
        let results = self.foundFaces.filter{ $0.name == name && $0.timestamp != frame.timestamp }
        ...
        guard let existentFace = results.first else {
            let node = SCNNode.init(withText: "", position: position)
            Async.main {
